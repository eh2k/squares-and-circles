...
$mkmodule $X.cpp \
    --no-opt \
    --build_flags="-fsingle-precision-constant -mfloat-abi=hard -mfpu=fpv5-d16 -ffast-math -DNDEBUG -O3 $BUILD_FLAGS -I. -I../lib/ " \
    --public-symbols="setup,process,display,screensaver,__ui_event_handler" \
    --name="$NAME" > $X.log
...
which elf-size-analyze && elf-size-analyze -t arm-none-eabi- ./$X.elf -F -R --no-color >> $X.log
